...
# outputのパス記述してくださいs
fnot = open("/Users/tAku/Nextremer/data/notFoundList.txt" , "w" , encoding = "utf-8")
...
for _file in files:
    if re.search(".txt",_file) != None:
        ...
        for line in lines:
            ...
            # outputのパス記述してください
            if p_only:
                fou = open('/Users/tAku/Nextremer/data/wikidata_raw_p_only/'+ line + '.txt','w', encoding = 'utf-8')
                html = opener.open(u).read()
                soup = BeautifulSoup(html, "lxml")
                text = soup.findAll(("p","h1","h2","h3","h4")) # 指定タグ内のテキスト全てをリスト格納
            else:
                fou = open('/Users/tAku/Nextremer/data/wikidata_raw_pNdd/'+ line + '.txt','w', encoding = 'utf-8')
                html = opener.open(u).read()
                soup = BeautifulSoup(html, "lxml")
            ...
            for part in text:
                if part.name in ["p", "dd"]:
                    ...
                    # sub = regex.sub(r"(?<!「[^。「」]*)。(?![^。「」]*」)", "。\n",part.get_text())
                    # sub = regex.sub(r"(?<!（[^。（）]*)。(?![^。（）]*）)", "。\n",part.get_text())
                    # sub = regex.sub(r"(?<rec>\[(?:[^\[\]]+|(?&rec))*\])|(?<rec2>\((?:[^\(\)]+|(?&rec2))*\))|(?<rec3>（(?:[^（）]+|(?&rec3))*）)|。|:|","",part.get_text())
                    #fou.write(part.get_text())
                    fou.write(lines+"\n")
                else:
                    if part.get_text() != "目次":
